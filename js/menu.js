...
// "class PaginaMenu extends PageBase" hereda de PageBase (la clase
// que está en storage.js), así que PaginaMenu ya trae gratis los
// métodos get(), redirect(), etc. sin tener que volver a escribirlos.
class PaginaMenu extends PageBase {
  ...
  mostrarDatosDelUsuario() {
    var nombreCompleto = this.get('nombreCompleto'); // "get" es un método HEREDADO de PageBase
  ...
  iniciar() {
    if (!this.user) {
      this.redirect('secion.html'); // "redirect" también es un método heredado de PageBase
      return;
    }
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (!api) return;
  var page = new PaginaMenu(api); // "new" crea el objeto real a partir de la clase
  page.iniciar();
});